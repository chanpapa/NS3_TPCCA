...
def Counterexp(exp_duration,packet_buffer_list,iteration,ccp_algs,delay_list,bdw_list):
            ...
            for predelay,postdelay in delay_list:        
                for ccp_alg in ccp_algs:
                    for iter_num in range(iteration):    
                        ...
                        sub= " \" trace_file=f'{trace_file}' filename=f'{filename}'  tcpTypeId =f'{ccp_alg}'  experimenttype=1 preBdw=f'{bdw}' preDelay=f'{predelay}' postDelay=f'{postdelay}' stopTime=f'{exp_duration}'  packetbuffer=f'{packet_buffer}' \" "
